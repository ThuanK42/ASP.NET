...
/*        private bool check_class(string maLH)
        {
            bool result;
            ...
            if (cmd.ExecuteScalar() != null) result = true; // ton tai tra ve true
            else result = false; // khong ton tai

            return result;
        }*/

        private bool Check_class_arrayList(string codeClass)
        {
            foreach (Classes t in listClasseses)
            {
                if (t.CodeClass == codeClass) return true;
            }
            return false;
        }
    }
